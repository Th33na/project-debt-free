...
def build_account_data(cc_info_df):
    ...
    guestimate_df = updated_cc_info.groupby(['user_id', 'card'],as_index = False)['amount'].sum()

    guestimate_df['credit_limit'] = guestimate_df.apply(lambda row: random_ceiling_on_cc_limit(row), axis=1)
    guestimate_df['card_name'] = 'CARD' + guestimate_df['card'].astype(str)
    guestimate_df['annual_percentage_rate'] = guestimate_df.apply(lambda row: random_apr(row), axis=1)
    guestimate_df.drop(columns=['amount'], inplace=True)
    guestimate_df['user_name'] = 'USER' + guestimate_df['user_id'].astype(str)
    guestimate_df['card_name'] = 'CARD' + guestimate_df['card'].astype(str)
    
    return guestimate_df
